...
def convert(input_docx: Path, output_pdf: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dk_sport_pdf_") as td:
        work = Path(td)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert the canonical Dr. Komorowski sport DOCX report to PDF without re-authoring.")
